...
def parse_warnings(analysis_dir, tool_config_data, raw_input_file=None, parsed_output_file=None):
    """This function handles parsing of raw SpotBugs data.

    Inputs:
        - analysis_dir: Absolute path to the raw SpotBugs output file directory [string]
    ...
    if raw_input_file is None:
        raw_input_file = analysis_dir.joinpath('spotbugs.sarif')

    # Set the output file
    if parsed_output_file is None:
        parsed_output_file = tool_config_data.get('raw_results_dir').joinpath('spotbugs_raw.scrub')

    # Print a status message
    logging.info('\t>> Executing command: get_spotbugs_warnings.parse_warnings(%s, %s)', analysis_dir,
                 parsed_output_file)
