...
def assert_scrubme_success(output_dir, language):
    ...
    log_files = glob.glob(helpers.test_tmp_dir + '/.scrub/log_files/*.log')
    filtering_log_file = os.path.normpath(output_dir + '/log_files/filtering.log')
...
def assert_mod_helper_success(output_dir, tool):
    # Initialize variables
    ...
    if tool == 'filter':
        assert open(filtering_log_file).read().count('') > 0
        assert open(filtering_log_file).read().count(' ERROR   ') == 0
        assert open(filtering_log_file).read().count('CommandExecutionError') == 0

    elif tool == 'collaborator':
        collaborator_log_file = glob.glob(output_dir + '/log_files/collaborator*')
